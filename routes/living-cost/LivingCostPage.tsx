...
export function LivingCostPage() {
  ...
  const [selectedCity, setSelectedCity] = useState('广州')
  const [mode, setMode] = useState('基础版')
  const lifestyle = mode === '基础版' ? 'normal' : 'luxury'
  const output = calculateLivingCost(mockScenario.living_cost.data, {
    city: selectedCity,
    monthly_income: 18000,
    lifestyle: lifestyle as 'normal' | 'luxury',
  })
  const bestCity = useMemo(() => cities.reduce((best, city) => {
    ...
    <Shell>
      ...
      <section className="grid gap-5 lg:grid-cols-[1fr_0.35fr]">
        <div>
          ...
          <ScenarioTabs items={cities.map((city) => city.city)} active={selectedCity} onChange={setSelectedCity} />
        </div>
        <div>
          <p className="mb-3 text-sm font-bold text-slate-600">消费模式</p>
          <ScenarioTabs items={['基础版', '舒适版']} active={mode} onChange={setMode} />
        </div>
      </section>
      ...
      <Panel className="mt-6 p-6">
        <div className="grid gap-6 xl:grid-cols-[1fr_0.45fr]">
          <section>
            <h2 className="mb-5 text-xl font-bold">月度生活成本对比（{mode}）</h2>
      ...
      <PageBottom
        summaries={[
          {title: '当前城市', text: `${selectedCity} 在 ${mode} 下月度支出约 ${money(output.monthly_total)}。`},
          {title: '成本最低', text: `${bestCity.city} 的综合生活成本最低。`},
          {title: '储蓄空间', text: `当前储蓄率约 ${output.saving_rate}%，适合持续积累。`},
          {title: '城市建议', text: '结合收入机会、通勤和生活质量综合选择。'},
        ]}
      />
    </Shell>
  )
...
function AudienceCard({title, text, tone}: {title: string; text: string; tone: 'blue' | 'green' | 'purple'}) {
  ...
  return (
    <section className="rounded-[8px] border border-slate-200 bg-white p-5">
      ...
      <Button type="button" variant="ghost" className="mt-4 h-auto p-0 text-sm font-bold text-indigo-600">查看建议 →</Button>
    </section>
  )
}
